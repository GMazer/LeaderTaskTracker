      ...
      <div className="space-y-4">
        {filteredTasks.length === 0 ? (
          ...
          filteredTasks.map(task => (
            <div 
              key={task.id} 
              className={`bg-white dark:bg-slate-800 rounded-xl border shadow-sm transition-all duration-200 overflow-hidden group ${expandedTaskId === task.id ? 'border-blue-300 dark:border-blue-700 ring-1 ring-blue-200 dark:ring-blue-900' : 'border-gray-100 dark:border-slate-700 hover:shadow-md'}`}
            >
                ...
                <div className="bg-gray-50 dark:bg-slate-700/30 border-t border-gray-100 dark:border-slate-700 p-4 animate-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="space-y-3 mb-4 max-h-60 overflow-y-auto custom-scrollbar">
                        {task.notes.length === 0 ? (
                            ...
                            task.notes.map(note => (
                                <div key={note.id} className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-gray-200 dark:border-slate-600 shadow-sm">
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 px-4 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                            autoFocus
                        />
                        <button
